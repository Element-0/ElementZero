...
/*~
 * @desc A class 'storing' strings into distinct, reusable compile-time types
 * that can be used as type parameters in a template parameter list.
 ...
 *       nonetheless.
 */
template <char... C> struct typestring final {
private:
...
template <char... C> constexpr char const typestring<C...>::vals[sizeof...(C) + 1];
 ...
 *       set the maximum length of the 'typestring' from 1 to 1024 (2^0 to
 * 2^10). Although all preprocessor implementations tested are capable of far
...
template <int N, int M> constexpr char tygrab(char const (&c)[M]) noexcept { return c[N < M ? N : M - 1]; }
...
template <char... X, char... Y> auto typoke(typestring<X...>, typestring<'\0'>, typestring<Y>...) -> typestring<X...>;
...
template <char... C> auto typeek(typestring<C...>) -> decltype(typoke(typestring<C>()...));
...
#define TYPESTRING4(n, x)                                                                                              \
  irqus::tygrab<0x##n##0>(x), irqus::tygrab<0x##n##1>(x), irqus::tygrab<0x##n##2>(x), irqus::tygrab<0x##n##3>(x)

/* 2^3 = 8 */
#define TYPESTRING8(n, x)                                                                                              \
  irqus::tygrab<0x##n##0>(x), irqus::tygrab<0x##n##1>(x), irqus::tygrab<0x##n##2>(x), irqus::tygrab<0x##n##3>(x),      \
      irqus::tygrab<0x##n##4>(x), irqus::tygrab<0x##n##5>(x), irqus::tygrab<0x##n##6>(x), irqus::tygrab<0x##n##7>(x)

/* 2^4 = 16 */
#define TYPESTRING16(n, x)                                                                                             \
  irqus::tygrab<0x##n##0>(x), irqus::tygrab<0x##n##1>(x), irqus::tygrab<0x##n##2>(x), irqus::tygrab<0x##n##3>(x),      \
      irqus::tygrab<0x##n##4>(x), irqus::tygrab<0x##n##5>(x), irqus::tygrab<0x##n##6>(x), irqus::tygrab<0x##n##7>(x),  \
      irqus::tygrab<0x##n##8>(x), irqus::tygrab<0x##n##9>(x), irqus::tygrab<0x##n##A>(x), irqus::tygrab<0x##n##B>(x),  \
      irqus::tygrab<0x##n##C>(x), irqus::tygrab<0x##n##D>(x), irqus::tygrab<0x##n##E>(x), irqus::tygrab<0x##n##F>(x)

/* 2^8 = 256 */
#define TYPESTRING256(n, x)                                                                                            \
  TYPESTRING16(n##0, x), TYPESTRING16(n##1, x), TYPESTRING16(n##2, x), TYPESTRING16(n##3, x), TYPESTRING16(n##4, x),   \
      TYPESTRING16(n##5, x), TYPESTRING16(n##6, x), TYPESTRING16(n##7, x), TYPESTRING16(n##8, x),                      \
      TYPESTRING16(n##9, x), TYPESTRING16(n##A, x), TYPESTRING16(n##B, x), TYPESTRING16(n##C, x),                      \
      TYPESTRING16(n##D, x), TYPESTRING16(n##E, x), TYPESTRING16(n##F, x)

#define typestring_is(x) decltype(irqus::typeek(irqus::typestring<TYPESTRING256(, x)>()))
#define strsig(x) irqus::typestring<TYPESTRING256(, x)>
