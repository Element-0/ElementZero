...
public:
  ...
  inline IdentityDefinition(ScoreboardId const &id, ActorUniqueID const &actor)
      : id(id), actor(actor), type(Type::Actor) {
    fake_name = std::to_string(actor.value);
    _init();
  }

  inline IdentityDefinition(ScoreboardId const &id, PlayerScoreboardId const &pid)
      : id(id), player_id(pid), type(Type::Player) {
    _init();
  }

  inline IdentityDefinition(ScoreboardId const &id, std::string const &fake)
      : id(id), fake_name(fake), type(Type::Fake) {
    _init();
  }

  inline IdentityDefinition(IdentityDefinition &&rhs)
      : id(std::move(rhs.id)), hidden_fake_player_type(std::move(rhs.hidden_fake_player_type)),
        player_id(std::move(rhs.player_id)), actor(std::move(rhs.actor)), fake_name(std::move(rhs.fake_name)),
        type(std::move(rhs.type)) {
    _init();
  }

  inline IdentityDefinition(IdentityDefinition const &rhs)
      : id(rhs.id), hidden_fake_player_type(rhs.hidden_fake_player_type), player_id(rhs.player_id), actor(rhs.actor),
        fake_name(rhs.fake_name), type(rhs.type) {
    _init();
  }
